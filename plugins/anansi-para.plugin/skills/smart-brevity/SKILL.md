...
description: >
  ...
  2022). Detects the input type — email, email thread, newsletter,
  workplace memo, meeting agenda/notes, speech, presentation, social media
  post, company-wide update, or generic prose — and applies matching
  type-specific rules layered over universal Smart Brevity principles.
  Use when the user says "smart brevity this", "/brevity", "rewrite in
  smart brevity", "tighten this up", "make this punchier", "apply smart
  brevity rules", or asks to compress prose into Smart Brevity form.
  Also handles TOC atomization mode: when the user provides both a source
  document and a PARA TOC with decimal-addressed sections and bracket type
  tags, generates one Smart Brevity block per TOC section, separated by
  --- for mechanical parsing into vault notes.
---
...
| **book** | Published book or long-form nonfiction with named chapters/sections; someone else's content being processed for retention |
...
3. Type-specific file:
   ...
   - `book.md` — book (reference outputs: `resources/smart-brevity.sb.md`, `resources/para-method.smart-brevity.md`)
...
- `### Edges` section: present only when relationships are inferable from the source. Omit entirely if none.
...
**`[person]` and `[organization]` blocks are strictly atomic identity records.** No role, active status, connected-to, context, or narrative content. What the person did, said, or contributed lives in the discussion blocks (Section 3). What they are affiliated with lives in graph edges — not in the identity note.

```
### [address] [title] [person]
One lead sentence — who this person is (name + single identifying fact).

- **Email:** contact_email (if available — always first)
- **Phone:** contact_phone (if available — always second)
- **Met via:** met_via (if available — always third)

---
```

```
### [address] [title] [organization]
One lead sentence — what this organization is.

- **Domain/Type:** fact (if available)
- **Met via:** context (if available)
...
```
### [address] [title] [note]
One lead sentence — the single most important fact about what this topic/document is.
...
```
### [address] [title] [project]
One lead sentence — what this project is and its outcome.
...
```
### [address] [title] [area]
One lead sentence — what domain of responsibility this area covers.

- **Key term:** fact

### Edges
- owned_by: person:[slug]
...
Rules:
- `[person]` blocks: Email, Phone, Met via ONLY — **no `### Edges` section**. Person is never a convergence point; relationships surface through the blocks of the things they're connected to.
- `[organization]` blocks: identity facts only (domain, type, met via) + `### Edges` for affiliated persons only — project/vendor relationships belong on the project block
- `[note]` blocks: key facts and Why it matters + `### Edges` for persons/orgs who touched this topic
- `[project]` blocks: outcome facts + `### Edges` for owners, vendors, contacts, referenced notes
- `[area]` blocks: responsibility scope + `### Edges` for ownership and supporting orgs
- Lead sentence for person: "[Full name] is [single identifying noun phrase]." Nothing more.
- Lead sentence for org: "[Org name] is a [type] [brief descriptor]." Nothing more.
- Drop `**Why it matters:**` for person and organization blocks entirely
- `### Edges` section: omit entirely if no relationships are inferable — do not emit an empty section
- Edge relationship verbs are not exhaustive — use the most precise verb inferable from the source (e.g., `presented_by`, `hired_by`, `introduced_via`, `co-authored_with`)
- Edge slugs must match the exact slug used in the entity's own block header
- `---` always present (after `### Edges` if present, otherwise after the last bullet)
- For `[person]`: always check resource-typer output and source email headers/signatures for Email and Phone before emitting
...
- [ ] `### Edges` present on all convergence blocks (organization, note, project, area) where relationships are inferable
- [ ] `[person]` blocks have NO `### Edges` section
- [ ] Edge slugs match the exact slug in the referenced entity's block header
- [ ] No empty `### Edges` sections — omit entirely if no edges found
...
### Edges
- built_by: organization:nous-research
- references: note:hermes-agent
...
### "Input is many pages long — someone else's content" (standard)

**Do not collapse.** When processing a book, article, transcript, or someone else's email thread, every chapter or major section gets its own Smart Brevity block. Nothing is dropped. Compression happens *within* each block — not by eliminating blocks.

**Format:** one block per chapter/section, `---` separated. Each block: tease or `## Section Title` header, lede, why it matters, bullets. Read the file in chunks using offset/limit if needed — process each section as you read it, never assume content you haven't read.

**Why:** The 200-words-per-item rule from the book applies to newsletter *items you are authoring* — you pick one big thing and write it up. When processing someone else's content, every unit carries information the reader needs. Dropping sections is lossy by definition, and Smart Brevity is short, not shallow.

### "Input is many pages long — you are the author" (standard)

When you are drafting something new (an email you're sending, a memo you're writing, a newsletter item, a speech), the One Big Thing rule applies: pick the single most important point, build ~200 words around it, cut everything that doesn't serve it. The 200-word cap is per item you create — not a license to discard someone else's content.
