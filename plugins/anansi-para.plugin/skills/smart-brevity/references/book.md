...
>
> Reference outputs: `resources/smart-brevity.sb.md` (direct SB pass on *Smart Brevity*), `resources/para-method.smart-brevity.md` (direct SB pass on *The PARA Method*)
...
Use when: capturing the book's editorial voice, hooks, and memorable lines. The output reads like a companion guide you'd want to re-read. Tease lines are punchy and opinionated, sometimes quoting the book directly.

Reference: `resources/smart-brevity.sb.md`

### Variant B — Chapter reference guide (encyclopedic)

Use when: producing a structured reference optimized for scanning and retrieval. Tease lines are functional labels. Part/section headers appear as non-content dividers. Output reads like a structured index.

Reference: `resources/para-method.smart-brevity.md`
...
| Bullets per chapter | 3–5 |
| Words per chapter block | ~100–200 |
| Total output (24-chapter book) | ~2,400–4,800 words |
| Reduction from source | 85–95% typical |
...
| Mistake | Fix |
|---|---|
| Collapsing the whole book to one block | Every chapter gets its own block — no exceptions |
| Skipping thin or repetitive chapters | Compress it to the minimum viable block; don't drop it |
| Tease that restates the chapter title | Tease is the takeaway, not the label — rewrite it |
| Lede that repeats the tease word-for-word | Lede adds; it never copies |
| Why it matters that repeats the lede | Add perspective, context, or consequence — not a restatement |
| Assuming content from chapters not yet read | Read every chapter; emit no block until you've read it |
| Missing `---` on final block | Always close with `---` |
| Part headers treated as content blocks | Part headers are dividers only — no lede, no bullets |
