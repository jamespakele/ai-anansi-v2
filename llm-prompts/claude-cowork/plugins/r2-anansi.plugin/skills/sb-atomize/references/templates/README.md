...
This directory is a **vendored mirror** of the canonical template library at `plugins/r2-anansi.plugin/references/templates/`. It holds all templates so this skill can run sandboxed without reaching outside its own directory. **Do not edit files here directly** — edit the canonical copy and re-sync.

Re-sync command (run from repo root):
```
CANONICAL="llm-prompts/claude-cowork/plugins/r2-anansi.plugin/references/templates"
DEST="llm-prompts/claude-cowork/plugins/r2-anansi.plugin/skills/sb-atomize/references/templates"
...
The plugin sandbox prevents skills from reaching files outside their own directory at runtime. So templates are **vendored**: copied from this canonical location into each consuming skill's `references/templates/` folder.
...
stable PARA primitives and the field set is bounded, so vendoring is unnecessary.

`para-process` is a pure orchestrator and reads no templates directly.

All mirrors must stay byte-identical to the canonical source. Verify with:

```
sha256sum **/templates/entity-area.md
sha256sum **/templates/entity-project.md
```

Each entity-type's hashes should collapse to a single value across all locations.
...
4. **Re-sync the mirror:**
   ...
   DEST="plugins/r2-anansi.plugin/skills/para-resource-entities/references/templates"
   cp "$CANONICAL"/*.md "$DEST/"
   ```
