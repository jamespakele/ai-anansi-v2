...
plugins/anansi.plugin/skills/sb-atomize/references/templates/              ← mirror (full library)
```
...
4. **Re-sync the mirrors:**
   ...
   for skill in para-resource-entities sb-atomize; do
     cp "$CANONICAL"/*.md "plugins/anansi.plugin/skills/$skill/references/templates/"
   done
   ```
