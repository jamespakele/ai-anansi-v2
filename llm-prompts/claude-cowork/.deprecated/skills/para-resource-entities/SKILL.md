...
Only `template_class: identity` templates are vendored here. The plugin's authoritative `references/templates/` carries three other classes: `source` (whole-document trees that get atomized), `content_unit` (the convergence floors where atomization stops — `meeting-topic-discussion.md` describes itself as *"the atomization floor for meeting-summary sources — do not decompose further"*), and `utility` (supporting shapes that ride alongside notes). None of those classes are about typing a Resource, so they're deliberately not vendored into this skill. See `references/templates/README.md` for the full breakdown of the four classes and the source → content_unit floor pairings.
...
3. **Load templates.** Read every `entity-*.md` file in this skill's `references/templates/` directory. Build the candidate type table; order most-specific first, `note` last.
...
6. **Identify Discussion sections.** Read the source's heading structure (or logical content blocks if the source is flat prose). Each substantive block that anchors at least one Resource gets a Section 3 line.
