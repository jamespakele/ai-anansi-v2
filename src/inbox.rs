...
async fn process_file(config: &Arc<Config>, pool: &DbPool, source_path: &Path) -> Result<()> {
    ...
    let filename = source_path.file_stem().and_then(|s| s.to_str()).unwrap_or("untitled");
    ...
    // Build LLM client
    let backend = config.inbox.llm_backend.as_deref().unwrap_or(&config.llm.backend);
    ...
    if !checkpoints_exist {
        eprintln!("[inbox] [{slug}] Stage 1 — para-process (reading skills from {})", skills_dir.display());

        let prompt_1a = match build_stage1a_prompt(skills_dir, &source_content, &source_id, &generated_at).await {
            Ok(p) => p,
            Err(e) => { fail_pipeline(&log_path, "skill_load_1a", &e.to_string()).await; return Err(e); }
        };
        let prompt_1b = match build_stage1b_prompt(skills_dir, &source_content, &source_id, &generated_at).await {
            Ok(p) => p,
            Err(e) => { fail_pipeline(&log_path, "skill_load_1b", &e.to_string()).await; return Err(e); }
        };
        ...
        let opts = InferOpts { temperature: 0.1, max_tokens: 8192, json_mode: false };
    ...
    let prompt_2 = match build_stage2_prompt(
        skills_dir, &source_content, &source_id, &generated_at,
        &source_title, &slug, &pa_toc, &pa_typed, &res_toc, &res_typed,
    ).await {
        Ok(p) => p,
        Err(e) => { fail_pipeline(&log_path, "skill_load_2", &e.to_string()).await; return Err(e); }
    };

    log_event(&log_path, serde_json::json!({"event":"stage_start","stage":"sb-atomize","ts":chrono::Utc::now().to_rfc3339()})).await;

    let opts2 = InferOpts { temperature: 0.15, max_tokens: 16384, json_mode: false };
    match llm.infer(&prompt_2, opts2).await.and_then(|o| parse_two_file_output(&o)) {
        ...
        Err(e) => {
            ...
            eprintln!("[inbox]   Stage 1 checkpoints preserved in: {}", archive_dir.display());
            return Err(e);
        }
    }
...
async fn load_skill(skills_dir: &Path, skill_subpath: &str) -> Result<String> {
    ...
    let skill_md = fs::read_to_string(&skill_md_path).await
        .with_context(|| format!(
            "reading skill file '{}'. \
