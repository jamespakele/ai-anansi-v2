...
4. **Re-sync the mirrors:**
   ```
   for dest in plugins/r2-anansi.plugin/skills/{para-pipeline,para-toc,resource-typer,para-resource-entities}/references/templates source/templates; do
     cp plugins/r2-anansi.plugin/references/templates/<template>.md "$dest/"
   done
   ```
